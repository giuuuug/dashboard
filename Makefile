##########################################################################################################################
# File automatically-generated by tool: [projectgenerator] version: [4.4.0-B60] date: [Thu Dec 05 19:11:53 CET 2024] 
##########################################################################################################################
...
######################################
# C sources
include $(shell pwd)/Lib/LVGL9/lvgl/lvgl.mk
C_SOURCES = $(CSRCS)
C_SOURCES += \
Core/Src/main.c \
Core/Src/gpio.c \
...
ADDITIONALLDFLAGS = -specs=nano.specs -u_printf_float -Wl,--print-memory-usage
...
$(BUILD_DIR)/$(TARGET)_shifted.elf: $(OBJECTS) | $(BUILD_DIR)
	$(CC) $(OBJECTS) $(MCU) $(ADDITIONALLDFLAGS) -TSTM32F446RETx_FLASH_shifted.ld $(LIBDIR) $(LIBS) -Wl,-Map=$(BUILD_DIR)/$(TARGET)_shifted.map,--cref -Wl,--gc-sections -o $@
